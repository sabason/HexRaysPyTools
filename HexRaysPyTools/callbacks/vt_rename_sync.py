...
def global_get_class_name(full_name):
    if "::" in full_name:
        ...
        # class_name = ida_struct.get_struc_name(sid).rsplit("_vtbl",1)[0]
        class_name = ""
        meth_name = full_name
...
def struct_get_class_name(sid,meth_member):
    meth_full_name = ida_struct.get_member_name(meth_member.id)
...
class VtMethodsRenameSync_hook(ida_idp.IDB_Hooks):
            ...
            for sid, meth_idx in vt_list:
                if type(sid) == int:
                    ...
                    if type(sid) == int and (exclude_sid is None or exclude_sid != sid):
                        meth_member = ida_struct.get_member(ida_struct.get_struc(sid), Const.EA_SIZE * meth_idx)
                        # struct_class_name, old_meth_name = struct_get_class_name(sid, meth_member)
                        self.renaming['struct'].append((sid, meth_member.id))
                        ida_struct.set_member_name(ida_struct.get_struc(sid), meth_member.soff, new_meth_name)

    def renamed(self, ea, new_name, local_name, old_name):
        if not ida_struct.is_member_id(ea) and ida_struct.get_struc(ea) is None:
    ...
    def renaming_struc_member(self, sptr, mptr, newname):
        logger.debug("Renaming_struct_member sid = 0x%08X (%s), mptr.soff = 0x%02X to '%s'"%(sptr.id, ida_struct.get_struc_name(sptr.id), mptr.soff, newname))
        sid = sptr.id
        if (sid, mptr.id) in self.renaming['struct']:
            logger.debug("Ignore sid = 0x%08X (%s), mptr.soff = 0x%02X" % (sid, ida_struct.get_struc_name(sptr.id), mptr.soff))
